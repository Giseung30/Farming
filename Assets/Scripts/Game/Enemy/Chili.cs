...
public class Chili : Enemy
{
    ...
    public void HPControl() //체력 관리하는 함수
    {
        if (HP <= 0 && !IsDeath) //체력이 다 떨어지면
        {
            ...
            PlayerScript.Kill(3); //플레이어의 킬 함수 실행
        }
    }
    ...
    public void FireNeedle() //바늘 발사하는 함수
    {
        GameObject NeedleClone = Instantiate(NeedleObject, NeedleObject.GetComponent<Transform>().position, NeedleObject.GetComponent<Transform>().rotation); //바늘 오브젝트 생성
        NeedleClone.SetActive(true); //바늘 오브젝트 활성화
